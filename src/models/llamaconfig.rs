...
pub fn load_llama3_2_1b<B: Backend>(
    ...
    let llama_config = LlamaConfig::llama3_2_1b(tokenizer_path)
        .with_max_seq_len(max_seq_len);
    let mut llama =  llama_config.init::<B, Tiktoken>(device)?;
    let cache_config = CacheConfig::from(llama_config);
...
pub fn load_llama3_1_8b<B: Backend>(
    ...
    let llama_config = LlamaConfig::llama3_1_8b(tokenizer_path)
        .with_max_seq_len(max_seq_len);
    let mut llama = llama_config.init::<B, Tiktoken>(device)?;
    let cache_config = CacheConfig::from(llama_config);
...
pub fn load_llama3_8b<B: Backend>(
    ...
    let llama_config = LlamaConfig::llama3_8b(tokenizer_path)
        .with_max_seq_len(max_seq_len);
...
pub fn llama3_2_1b_pretrained_tiktoken<B: Backend>(
    max_seq_len: usize,
    device: &Device<B>,
) -> Result<(Llama<B, Tiktoken>, CacheConfig), String> {
    // Llama-3.2 models support context length up to 128K tokens.
...
pub fn llama3_1_8b_pretrained_tiktoken<B: Backend>(
    max_seq_len: usize,
    device: &Device<B>,
) -> Result<(Llama<B, Tiktoken>, CacheConfig), String> {
...
pub fn llama3_8b_pretrained_tiktoken<B: Backend>(
    max_seq_len: usize,
    device: &Device<B>,
) -> Result<(Llama<B, Tiktoken>, CacheConfig), String> {
...
impl LlamaConfig {
    ...
    pub fn llama3_2_3b(tokenizer_path: &str) -> Self {
        ...
        Self::new(8192, 128256)
            .with_tokenizer(tokenizer_path.to_string())
            .with_d_model(3072)
